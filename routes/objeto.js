...
router.put('/',verifyToken,(req, res)=>{
    if(req.query.id){
        Objeto.findOneAndUpdate({'_id':req.query.id},{'name':req.body.name,'idcaja':req.body.idcaja,'description':req.body.description},{new:true},(err,doc)=>{
            if(err) return res.status(500).send({msg:err})
